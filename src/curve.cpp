...
void BezierCurve::drawCurves(ArgParser *args)
{
    ...
    for (int i = 0; i < num_vertices - 3; i += 3)
    {
        Matrix G;
        for (int j = 0; j < 4; ++j) {
            G.Set(j, 0, vertices[i+j].x());
            G.Set(j, 1, vertices[i+j].y());
            G.Set(j, 2, vertices[i+j].z());
        }
...
void BezierCurve::OutputBSpline(FILE *file) {
    ...
    for (int i = 0; i < num_vertices; ++i) {
        G.Set(i, 0, vertices[i].x());
        G.Set(i, 1, vertices[i].y());
        G.Set(i, 2, vertices[i].z());
    }
    Matrix inv_BSpline_Matrix = BSpline_Matrix;
    inv_BSpline_Matrix.Inverse();
    Matrix G_BSpline = G * Bezier_Matrix * inv_BSpline_Matrix;
    for (int i = 0; i < num_vertices; ++i) {
...
void BSplineCurve::OutputBezier(FILE *file) {
    ...
    Matrix G_Bezier = G * BSpline_Matrix * inv_Bezier_Matrix;
    for (int i = 0; i < num_vertices; ++i) {
